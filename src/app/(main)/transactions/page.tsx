...
export default function TransactionsPage() {
    ...
    const handleSell = async (e: React.FormEvent) => {
        ...
        const res = await fetch("/api/transactions", {
            ...
            body: JSON.stringify({ shopId: activeStore._id, itemId: selectedItem.itemId?._id, amountSold: Number(sellAmount) })
        });

        if (res.ok) {
            const { shop: updatedShop } = await res.json();
            // Need to populate it ideally, or just decrement local state for UX speed
            setActiveStore((prev: any) => {
                const newInv = prev.inventory.map((i: any) => {
	...
	return (
		<div className="w-full h-full flex flex-col gap-6 px-4 md:px-8 py-6 pb-24 overflow-y-auto scrollbar-hidden">
			<div className="flex flex-col gap-2 border-b border-theme-border/50 pb-6 pt-2">
				...
				<p className="text-theme-text/50">Sell items from <span className="text-theme-accent font-bold">{activeStore?.title || "your assigned store"}</span>.</p>
			</div>

            {activeStore ? (
                <motion.div className="flex flex-col gap-6 w-full mt-2">
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                        {activeStore.inventory?.map((inv: any, idx: number) => (
                            <motion.button 
                            ...
                            >
                                ...
                                <div className="flex flex-col flex-1 w-full relative">
                                    <span className="font-bold text-xl truncate max-w-full text-theme-text group-hover:text-theme-accent transition-colors">{inv.itemId?.name || "Item"}</span>
                                    <div className="flex items-center justify-center gap-2 text-theme-text/60 text-sm mt-3 font-medium">
                                        <span className="bg-theme-background px-3 py-1 rounded-full"><span className="text-theme-text">{inv.amount}</span> in stock</span>
                                        <span className="bg-emerald-500/10 text-emerald-400 px-3 py-1 rounded-full">${inv.itemId?.unitPrice} ea</span>
                                    </div>
                                </div>
                            </motion.button>
                        ))}
                    ...
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
                        <motion.div initial={{ scale: 0.95, y: 10 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.95, y: 10 }} className="bg-theme-card p-6 rounded-3xl w-full max-w-sm shadow-2xl border border-theme-border/50">
                            ...
                            <form onSubmit={handleSell} className="flex flex-col gap-4">
                                <label className="text-xs uppercase tracking-wider font-bold text-theme-text/60 px-1">Quantity</label>
                                <input 
                                    value={sellAmount} 
                                ...
                                <div className="flex flex-col gap-3 mt-4">
                                    <button type="submit" className="w-full py-4 rounded-xl bg-theme-accent text-theme-background font-black text-lg tracking-wide hover:opacity-90 transition-opacity">Confirm Sale</button>
                                    <button type="button" onClick={() => setShowSellModal(false)} className="w-full py-3 rounded-xl hover:bg-theme-background text-theme-text/60 hover:text-theme-text font-semibold transition-colors">Cancel</button>
                                </div>
                            </form>
                        </motion.div>
                    </motion.div>
